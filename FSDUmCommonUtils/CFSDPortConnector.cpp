...
HRESULT CFSDPortConnector::SendMessage(
    ...
    hr = FilterSendMessage(
        ...
        dwInBufferSize, 
        lpReplyBuffer, 
        lpReplyBufferSize ? *lpReplyBufferSize : 0,
        &dwBytesReturned);
    RETURN_IF_FAILED(hr);
...
HRESULT CFSDPortConnector::RecieveMessage(CFSDPortConnectorMessage * pMessage)
{
    ...
    memset(&pMessage->aOverlapped, 0, sizeof(pMessage->aOverlapped));

    hr = FilterGetMessage(
        m_hPort, 
        &pMessage->aRecieveHeader, 
        FIELD_OFFSET(CFSDPortConnectorMessage, aOverlapped), 
        &pMessage->aOverlapped);
    RETURN_IF_FAILED(hr);
